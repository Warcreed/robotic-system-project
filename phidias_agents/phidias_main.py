...
# reactors interpreted by the blocks_manager agent
...
# beliefs sent by the blocks_manager agent
...
class main(Agent):
    def main(self):
      ...
      generate(N) / (geq(N,1) & leq(N, 6)) >> [ 
        ...
        +gen(N)[{'to': 'blocks_manager@127.0.0.1:6767'}] 
      ]
      ...
      +is_block_present(True)[{'from': _from}] / target(N) >> [
        ...
        +block_slot_status(N, True)[{'to': 'blocks_manager@127.0.0.1:6767'}],
      ...
      +is_block_present(False)[{'from': _from}] / target(N) >> [
        ...
        +block_slot_status(N, False)[{'to': 'blocks_manager@127.0.0.1:6767'}],
        "N=N+1",
        _scan_block_slot(N)
      ]
      ...
      +color(C)[{'from':_from}] / target(N) >> [ 
        ...
        +block_slot_color(N, C)[{'to': 'blocks_manager@127.0.0.1:6767'}],
        "N=N+1",
        _scan_block_slot(N)
      ]
